...
$sentencia = $base_de_datos->prepare("INSERT INTO servicios_vendidos(id_servicio, id_venta, cantidad) VALUES (?, ?, ?);");
$sentenciaExistencia = $base_de_datos->prepare("UPDATE servicios SET disponible = disponible - ? WHERE id = ?;");
foreach ($_SESSION["carrito"] as $servicio) {
	$total += $servicio->total;
	$sentencia->execute([$servicio->id, $idVenta, $servicio->cantidad]);
